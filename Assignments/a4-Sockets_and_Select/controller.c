...
int process_message(struct cignal *cig, int *device_record) {
    ...
    // Check cig is valid (type is handshake or update)
    if (is_valid_type(cig) == 1) {
        // Check device is registered or not
        if (is_registered(cig->hdr.device_id, device_record) == 1) {
            // Device is registered
            // cig header type is handshake -> error
            if (cig->hdr.type == HANDSHAKE) {
                fprintf(stderr, "This device is already regiestered! The message is discarded\n");
                return -1;
            } 
            // cig header type is update -> update
            else {
                // cig device type is temperature
                if (cig->hdr.device_type == TEMPERATURE) {
                    printf("Temperature: %.4f --> Device_ID: %d\n", cig->value, cig->hdr.device_id);
                } 
                // cig device type is humidity
                else {
                    printf("Humidity: %.4f --> Device_ID: %d\n", cig->value, cig->hdr.device_id);
                }
                // Adjust fan
        ...
        // Device is unregistered
            ...
            // cig header type is handsake -> register
            if (cig->hdr.type == HANDSHAKE) {
                int new_id = register_device(device_record);
                cig->hdr.device_id = new_id;
                device_record[new_id - LOWEST_ID] = 1;
            } 
            // cig header type is update -> error
            else {
                fprintf(stderr, "Received update cignal from unregistered device! The message is discarded\n");
                return -1;
            }
        }
    printf("********************END EVENT********************\n\n");
